...
impl Alphametics {
    ...
    fn solve_inner(&mut self, char_index: usize) -> Option<()> {
        if char_index == self.chars.len() {
            return Some(()); // we've run out of letters. State is NOT INvalid
        }
        let current = self.chars[char_index];
        ...
        for current_num in avail_nums {
            ...
            if self.validate() {
                ...
                if self.solve_inner(char_index + 1).is_some() {
