...
// core logic remains same. The `validate` function uses `col_char_count`
//  to return early if all chars for the column are not assigned.
//  that shaved time for the `test_puzzle_with_ten_letters` from 4.5ms to 2.7ms
...
struct Alphametics {
    ...
    c2n: HashMap<char, u8>,     // char-to-number mapping. This is the solution candidate
...
impl Alphametics {
    ...
    fn validate(&mut self) -> bool {
        ...
        let mut sum = 0u32;
        let mut mapped_number = 0u32;
        for (column, &col_char_count) in self.columns.iter().zip(self.col_char_count.iter()) {
            if col_char_count > self.c2n.len() {
                // this column is not fully assigned, so skip all remaining checks
                return true;
            }
            for ch in column.iter() {
                // note: last mapped_number is the expected result
                //  so, later, we'll have to subtract it from sum
                mapped_number = *self.c2n.get(ch).unwrap() as u32;
                sum += mapped_number;
            }
            ...
            sum -= mapped_number;
            let expected_result = mapped_number;
            if sum % 10 != expected_result {
