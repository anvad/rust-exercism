...
pub fn solve(input: &str) -> Option<HashMap<char, u8>> {
    ...
    println!("alphametics1= {:?}", alphametics);
    let res = alphametics.solve_inner();
    println!("alphametics2= {:?}", alphametics);
    if res.is_none() {
        return None;
    }
    Some(
...
impl Alphametics {
    ...
    fn validate(&mut self) -> bool {
        ...
        let mut sum = 0u32;
        for (expected_ch, (&col_char_count, col_map)) in self
            .solution_chars
            .iter()
            .zip(self.col_char_count.iter().zip(self.cols_map.iter()))
        {
            if col_char_count > self.c2n.len() {
                // this column is not fully assigned, so skip all remaining checks
                return true;
            }
            for (ch, &count) in col_map.iter() {
            ...
            let expected_sum = *self.c2n.get(expected_ch).unwrap();
            sum -= expected_sum;
