...
fn can_use_different_words_with_the_same_name_2() {
    ...
    assert!(f.eval(": foo dup ;").is_ok());
    assert!(f.eval(": bar foo ;").is_ok());
    assert!(f.eval(": baz dup bar ;").is_ok());
    assert!(f.eval(": foo 6 ;").is_ok());
    assert!(f.eval("1 baz bar foo").is_ok());
    assert_eq!(vec![1, 1, 1, 1, 6], f.stack());
}

#[test]
#[ignore]
fn can_use_different_words_with_the_same_name_3() {
    let mut f = Forth::new();
    assert!(f
        .eval(": foo dup ; : bar foo ; : foo 3 ; 1 bar foo 5")
        .is_ok());
    assert_eq!(vec![1, 1, 3, 5], f.stack());
}
